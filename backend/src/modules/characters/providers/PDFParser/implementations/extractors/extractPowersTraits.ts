...
export default function extractPowersTraits(
  line: string,
  creature: string,
    ...
    case 'Mortal':
      {
        ...
        const vampireDisciplines = [
          ...
          'Geomancy: Elemental Mastery',
          'Geomancy: Focused Mind',
          'Geomancy: The Green Path',
          'Geomancy: Hearth Path',
          'Geomancy: Movement of the Mind',
          "Geomancy: Neptune's Might",
          'Geomancy: Oneiromancy',
          'Geomancy: Path of Blood',
          'Geomancy: Path of Conjuring',
          'Geomancy: Path of Geomancy',
          'Geomancy: Path of Strands',
          'Geomancy: Spirit Manipulation',
          'Geomancy: Spirit Thaumaturgy',
          'Geomancy: Weather Control',
    ...
    case 'Mage':
      {
        ...
        if (power !== '') {
          const myPower: CharacterTrait =
            currPowersList.find(sphere => sphere.trait === power) ||
            ({
              trait: power,
              level: 0,
              type: 'powers',
          ...
          if (isAPower) {
            if (newPower) {
              ...
              currPowersList = currPowersList.map(sphere =>
                sphere.trait === power ? myPower : sphere,
              );
            }
          }
        }
    ...
    case 'Wraith':
      {
        ...
        if (power !== '') {
          const myPower: CharacterTrait =
            currPowersList.find(arcanoi => arcanoi.trait === power) ||
            ({
              trait: power,
              level: 0,
              type: 'powers',
          ...
          if (isAPower) {
            if (newPower) {
              ...
              currPowersList = currPowersList.map(arcanoi =>
                arcanoi.trait === power ? myPower : arcanoi,
              );
            }
          }
        }
